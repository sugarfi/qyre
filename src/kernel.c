...
void kmain(void) {
    ...
    node_database_header_t header2 = {
        .type = 1,
        .max_regions = MAX_REGION,
        .max_backups = MAX_BACKUP,
        .next_data_pos = 0
    };
    memcpy("BEGAYDOCRIME", header2.signature, 12);
    memcpy("memory__000000000000000000000001", header2.ref, 32);

    memset((char *) HEADER_ADDR2, 0, 512);
    memcpy((char *) &header2, (char *) HEADER_ADDR2, sizeof(node_database_header_t));

    for (region_set = (node_region_entry_t *) REGION_LIST_ADDR2, i = 0; i < MAX_REGION; region_set++, i++) {
        region_set->start = i * 32;
        region_set->end = i * 32 + 32;

        int j;
        node_t *node_set;
        for (node_set = (node_t *) NODE_LIST_ADDR2 + i, j = 0; j < NODES_PER_REGION; node_set++, j++) {
            node_set->id = 0;
            node_set->region = 0;
            node_set->backup = 0;
            node_set->type = 0;
    ...
    for (backup_set = (node_backup_entry_t *) BACKUP_LIST_ADDR2, i = 0; i < MAX_BACKUP; backup_set++, i++) {
        backup_set->type = 0;
        memset(backup_set->ref, 0, 32);
        memset((char *) backup_set->other, 0, 512);
    }
    ...
    node_database_t db2 = {
        .header = (node_database_header_t *) HEADER_ADDR2,
        .regions = (node_region_entry_t *) REGION_LIST_ADDR2,
        .backups = (node_backup_entry_t *) BACKUP_LIST_ADDR2,
        .nodes = (node_t *) NODE_LIST_ADDR2,
        .data = (uint8_t *) DATA_ADDR2
    };

    node_backup_entry_t db_ref2 = {
        .type = 1,
    };
    node_backup_memory_t db_other2 = {
        .addr = HEADER_ADDR2
    };
    memcpy(db2.header->ref, db_ref2.ref, 32);
    memset((char *) db_ref2.other, 0, 512);
    memcpy((char *) &db_other2, (char *) db_ref2.other, sizeof(node_backup_memory_t));
    db.backups[0] = db_ref2;

    node_ref_t ref;
    node_add((uint8_t *) "hello", 6, 0, &db2, &ref);
    node_t node;
    node_lookup(ref, db, &node);
    debug_printf("%s\r\n", (char *) db2.data + node.data_pos);
